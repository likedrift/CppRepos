...
void CloudiskServer::mysqlCallbackForRegister(wfrest::HttpResp * resp, WFMySQLTask * mysqltask){
    // loadUserRegisterModule mysqlCallbackForRegister
    // 0. 对任务的状态进行检测
...
void CloudiskServer::mysqlCallbackForLogin(string & encodedPassword, string & username, string & salt, string & mysqlurl, wfrest::HttpResp * resp, WFMySQLTask * mysqltask, SeriesWork *series){
    // 0. 对任务的状态进行检测
    int state = mysqltask->get_state();
    int error = mysqltask->get_error();
    if (state != WFT_STATE_SUCCESS)
...
void CloudiskServer::mysqlCallbackForInfo(string & username, wfrest::HttpResp * resp, WFMySQLTask * mysqltask){
    //...检测
    using namespace protocol;
    auto mysqlResp = mysqltask->get_resp();
    MySQLResultCursor cursor(mysqlResp);
...
void CloudiskServer::mysqslCallbackForQuery(wfrest::HttpResp * resp, WFMySQLTask * mysqltask){
    //...检测
    using namespace protocol;
    auto mysqlResp = mysqltask->get_resp();
    MySQLResultCursor cursor(mysqlResp);
...
void CloudiskServer::loadUserLoginModule()
{
    _httpserver.POST("/user/signin", [](const HttpReq *req, HttpResp * resp, SeriesWork * series){
        if(req->content_type() == APPLICATION_URLENCODED) {
            ...
            string mysqlurl("mysql://root:123@localhost");
            auto mysqlTask = WFTaskFactory::create_mysql_task(mysqlurl, 1, std::bind(&CloudiskServer::mysqlCallbackForLogin, 
                                                encodedPassword, username, salt, mysqlurl, resp, std::placeholders::_1, series));
...
void CloudiskServer::loadUserInfoModule()
{
    _httpserver.GET("/user/info", [](const HttpReq *req, HttpResp * resp, SeriesWork * series){
        ...
        auto mysqlTask = WFTaskFactory::create_mysql_task(mysqlurl, 1, 
                        std::bind(&CloudiskServer::mysqlCallbackForInfo, username, resp, std::placeholders::_1));
...
void CloudiskServer::loadFileQueryModule()
{
    _httpserver.POST("/file/query", [](const HttpReq *req, HttpResp * resp, SeriesWork * series){
        ...
        string mysqlurl("mysql://root:123@localhost");
        auto mysqlTask = WFTaskFactory::create_mysql_task(mysqlurl, 1, 
                        std::bind(&CloudiskServer::mysqslCallbackForQuery, resp, std::placeholders::_1));
